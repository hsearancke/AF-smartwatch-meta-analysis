...
model <- reitsma(data)
# Step 5: PRINT RESULTS (this is the key step)
summary(model)
packageVersion("mada")
fpr <- 0.175
fpr_lower <- 0.133
...
mada_matrix <- as.matrix(data_study[, c("TP", "FN", "FP", "TN")])
hsroc_model <- reitsma(mada_matrix)
summary(hsroc_model)
plot(hsroc_model)
plot(hsroc_model, sroclwd = 2)
points(
1 - (data_study$TN / (data_study$TN + data_study$FP)),  # FPR
data_study$TP / (data_study$TP + data_study$FN),        # Sensitivity
pch = 16,
col = "blue"
)
...
# Save Deeks funnel plot as Windows Metafile (.wmf)
win.metafile("deeks_funnel_plot.wmf", width = 6, height = 6)
plot(
data_cc$inv_sqrt_ESS,
data_cc$logDOR,
xlab = "1 / sqrt(Effective Sample Size)",
...
dev.off()
# -------------------------------
# 6. HSROC / SROC curve with study points
# -------------------------------
# Overall bivariate model using included subgroup data
handling_model <- reitsma(data_grouped_clean)
# Plot HSROC/SROC curve
plot(
  handling_model,
  sroclwd = 2,
  main = "HSROC Curve by Data Handling Method"
)
win.metafile("handling_hsroc_plot.wmf", width = 6, height = 6)
plot(
  handling_model,
  sroclwd = 2,
  main = "HSROC Curve by Data Handling Method"
